# 1 "/Users/juitenghsu/enzo-e/src/Enzo/hydro-mhd/ppml_fortran/PPML_MAIN.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/Users/juitenghsu/enzo-e/src/Enzo/hydro-mhd/ppml_fortran/PPML_MAIN.F"
...
# 4 "/Users/juitenghsu/enzo-e/src/Enzo/hydro-mhd/ppml_fortran/PPML_MAIN.F" 2

      Subroutine PPML(dn,vx,vy,vz,bx,by,bz,
     &     dnrx,vxrx,vyrx,vzrx,bxrx,byrx,bzrx,         
     &     dnry,vxry,vyry,vzry,bxry,byry,bzry,         
     &     dnrz,vxrz,vyrz,vzrz,bxrz,byrz,bzrz,
     &     dt, dx,dy,dz,
     &     nx, ny, nz,
...
c     dn,vx,vy,vz,bx,by,bz               - central primitive variables
c     dnrx,vxrx,vyrx,vzrx,bxrx,byrx,bzrx - x-right primitive variables
c     dnry,vxry,vyry,vzry,bxry,byry,bzry - y-right primitive variables
c     dnrz,vxrz,vyrz,vzrz,bxrz,byrz,bzrz - z-right primitive variables 
c     f1,f2,f3,f4,f5,f6,f7               - x-fluxes
c     g1,g2,g3,g4,g5,g6,g7               - y-fluxes
c     h1,h2,h3,h4,h5,h6,h7               - z-fluxes 

      Integer nx,ny,nz,ighost,jghost,kghost,nsubgrids
      Integer nxb,nxe,nyb,nye,nzb,nze 
      Integer i1,j1,k1,i2,j2,k2
      Integer nxp,nyp,nzp,i,j,k,n,idim,offset
     ...
     &     bzindex(nsubgrids*6)
      ...
      real (kind=8) dnrx(nx,ny,nz)           
      real (kind=8) vxrx(nx,ny,nz),vyrx(nx,ny,nz),vzrx(nx,ny,nz)           
      real (kind=8) bxrx(nx,ny,nz),byrx(nx,ny,nz),bzrx(nx,ny,nz)           

      real (kind=8) dnry(nx,ny,nz)           
      real (kind=8) vxry(nx,ny,nz),vyry(nx,ny,nz),vzry(nx,ny,nz)           
      real (kind=8) bxry(nx,ny,nz),byry(nx,ny,nz),bzry(nx,ny,nz)           

      real (kind=8) dnrz(nx,ny,nz)           
      real (kind=8) vxrz(nx,ny,nz),vyrz(nx,ny,nz),vzrz(nx,ny,nz)           
      real (kind=8) bxrz(nx,ny,nz),byrz(nx,ny,nz),bzrz(nx,ny,nz)           

      real (kind=8) f1(nx,ny,nz)           
      real (kind=8) f2(nx,ny,nz),f3(nx,ny,nz),f4(nx,ny,nz)           
      real (kind=8) f5(nx,ny,nz),f6(nx,ny,nz),f7(nx,ny,nz)           

      real (kind=8) g1(nx,ny,nz)           
      real (kind=8) g2(nx,ny,nz),g3(nx,ny,nz),g4(nx,ny,nz)           
      real (kind=8) g5(nx,ny,nz),g6(nx,ny,nz),g7(nx,ny,nz)           

      real (kind=8) h1(nx,ny,nz)           
      real (kind=8) h2(nx,ny,nz),h3(nx,ny,nz),h4(nx,ny,nz)           
      real (kind=8) h5(nx,ny,nz),h6(nx,ny,nz),h7(nx,ny,nz)           

      real (kind=8) ex(nx,ny,nz),ey(nx,ny,nz),ez(nx,ny,nz)           
      ...
      real (kind=8) Qp1(nx,7),Qr1(nx,7)
      real (kind=8) Qdy1(nx,7),Qdz1(nx,7)
      real (kind=8) Qlx1(nx,7),Qrx1(nx,7),Qvl1(nx,7),Qvr1(nx,7)
      real (kind=8) Qrp1(nx,7),Fm1(nx,7)
      real (kind=8) dx1(nx),dx2(nx)
      
      real (kind=8) Qp2(ny,7),Qr2(ny,7)
      real (kind=8) Qdx2(ny,7),Qdz2(ny,7)
      real (kind=8) Qlx2(ny,7),Qrx2(ny,7),Qvl2(ny,7),Qvr2(ny,7)
      real (kind=8) Qrp2(ny,7),Fm2(ny,7)
      real (kind=8) dy1(ny),dy2(ny)

      real (kind=8) Qp3(nz,7),Qr3(nz,7)
      real (kind=8) Qdx3(nz,7),Qdy3(nz,7)
      real (kind=8) Qlx3(nz,7),Qrx3(nz,7),Qvl3(nz,7),Qvr3(nz,7)
      real (kind=8) Qrp3(nz,7),Fm3(nz,7)
      real (kind=8) dz1(nz),dz2(nz)

      real (kind=8) df(nx,ny),vf(nx,ny),uf(nx,ny),wf(nx,ny)
      real (kind=8) bf(nx,ny),tf(nx,ny),pf(nx,ny)
      real (kind=8) dg(ny,nz),vg(ny,nz),ug(ny,nz),wg(ny,nz)
      real (kind=8) bg(ny,nz),tg(ny,nz),pg(ny,nz)          
      real (kind=8) dh(nz,nx),vh(nz,nx),uh(nz,nx),wh(nz,nx)
      real (kind=8) bh(nz,nx),th(nz,nx),ph(nz,nx)          
      
      real (kind=8) vax,vay,vaz,cga,cgs
      real (kind=8) dn1,dn2,vx1,vx2,vy1,vy2,vz1,vz2
      real (kind=8) bx1,bx2,by1,by2,bz1,bz2

      real (kind=8) epsk,v2k,exc,dexy,dexy1,dexy2,exi1,exi2,exk1,exk2
      real (kind=8) Vleer,dsc,dx,dy,dz,dt 
      ...
      Do k=nzb+1,nze-1
         Do j=nyb+1,nye-1
            ...
            Do i=nxb,nxe
               ...
               Qdy1(i,1) =
     +              VLeer (dn(i,j+1,k)-dn(i,j,k),dn(i,j,k)-dn(i,j-1,k))
               Qdy1(i,2) = 
     +              VLeer (vx(i,j+1,k)-vx(i,j,k),vx(i,j,k)-vx(i,j-1,k))
               Qdy1(i,3) =
     +              VLeer (vy(i,j+1,k)-vy(i,j,k),vy(i,j,k)-vy(i,j-1,k))
               Qdy1(i,4) =
     +              VLeer (vz(i,j+1,k)-vz(i,j,k),vz(i,j,k)-vz(i,j-1,k))
               Qdy1(i,5) =
     +              VLeer (bx(i,j+1,k)-bx(i,j,k),bx(i,j,k)-bx(i,j-1,k))
               Qdy1(i,6) =
     +              VLeer (by(i,j+1,k)-by(i,j,k),by(i,j,k)-by(i,j-1,k))
               Qdy1(i,7) =
     +              VLeer (bz(i,j+1,k)-bz(i,j,k),bz(i,j,k)-bz(i,j-1,k))

               Qdz1(i,1) =
     +              VLeer(dn(i,j,k+1)-dn(i,j,k),dn(i,j,k)-dn(i,j,k-1))
               Qdz1(i,2) =
     +              VLeer(vx(i,j,k+1)-vx(i,j,k),vx(i,j,k)-vx(i,j,k-1))
               Qdz1(i,3) =
     +              VLeer(vy(i,j,k+1)-vy(i,j,k),vy(i,j,k)-vy(i,j,k-1))
               Qdz1(i,4) =
     +              VLeer(vz(i,j,k+1)-vz(i,j,k),vz(i,j,k)-vz(i,j,k-1))
               Qdz1(i,5) =
     +              VLeer(bx(i,j,k+1)-bx(i,j,k),bx(i,j,k)-bx(i,j,k-1))
               Qdz1(i,6) =
     +              VLeer(by(i,j,k+1)-by(i,j,k),by(i,j,k)-by(i,j,k-1))
               Qdz1(i,7) =
     +              VLeer(bz(i,j,k+1)-bz(i,j,k),bz(i,j,k)-bz(i,j,k-1))
            ...
            Call Monot(1,nxp,Dx1,Dx2,qp1,qr1,qlx1,qrx1)
            ...
            CALL QDD6(nxp,Qrx1,Qlx1,Qvr1,Qvl1,Qp1)
            ...
            CALL POTOKx(nxp,Dx,Dt,Qvl1,Qvr1,Qp1,Qdy1,Qdz1,Qrp1,Fm1)
...
c--------------------------------------------------------
      ...
      Do k=nzb+1,nze-1
         Do i=nxb+1,nxe-1
            ...
            Do j=nyb,nye
               ...
               Qdx2(j,1) =
     +              VLeer(dn(i+1,j,k)-dn(i,j,k),dn(i,j,k)-dn(i-1,j,k))
               Qdx2(j,2) =
     +              VLeer(vx(i+1,j,k)-vx(i,j,k),vx(i,j,k)-vx(i-1,j,k))
               Qdx2(j,3) =
     +              VLeer(vy(i+1,j,k)-vy(i,j,k),vy(i,j,k)-vy(i-1,j,k))
               Qdx2(j,4) =
     +              VLeer(vz(i+1,j,k)-vz(i,j,k),vz(i,j,k)-vz(i-1,j,k))
               Qdx2(j,5) =
     +              VLeer(bx(i+1,j,k)-bx(i,j,k),bx(i,j,k)-bx(i-1,j,k))
               Qdx2(j,6) =
     +              VLeer(by(i+1,j,k)-by(i,j,k),by(i,j,k)-by(i-1,j,k))
               Qdx2(j,7) =
     +              VLeer(bz(i+1,j,k)-bz(i,j,k),bz(i,j,k)-bz(i-1,j,k))
     ...
     +              VLeer(bz(i,j,k+1)-bz(i,j,k),bz(i,j,k)-bz(i,j,k-1))
            ...
            Call Monot(2,nyp,Dy1,Dy2,qp2,qr2,qlx2,qrx2)
            ...
            CALL QDD6(nyp,Qrx2,Qlx2,Qvr2,Qvl2,Qp2)
            ...
            CALL POTOKy(nyp,Dx,Dt,Qvl2,Qvr2,Qp2,Qdx2,Qdz2,Qrp2,Fm2) 
...
c--------------------------------------------------------
     ...
     +              VLeer(bz(i,j+1,k)-bz(i,j,k),bz(i,j,k)-bz(i,j-1,k))
            ...
            Call Monot(3,nzp,Dz1,Dz2,Qp3,Qr3,qlx3,qrx3)
            ...
            CALL QDD6(nzp,Qrx3,Qlx3,Qvr3,Qvl3,Qp3)
            ...
            CALL POTOKz(nzp,Dx,Dt,Qvl3,Qvr3,Qp3,Qdx3,Qdy3,Qrp3,Fm3) 
...
C     !-----------------------------------------------------------------
            ...
            DO I=i1,i2
               ...
               if(abs(v2k).gt.epsk)then
                  ...
                  if(v2k.gt.0.)then
                     ...
                     exc=-(vy(i,j,k)*bz(i,j,k)-vz(i,j,k)*by(i,j,k))

                     dexy=-g7(i,j,k)-exc

                  else 

                     exc=-(vy(i,j,k+1)*bz(i,j,k+1)
     +                    -vz(i,j,k+1)*by(i,j,k+1))
                  ...
                  exc=-(vy(i,j,k+1)*bz(i,j,k+1)-vz(i,j,k+1)*by(i,j,k+1))
               ...
               if(abs(v2k).gt.epsk)then
                  ...
                  if(v2k.gt.0.)then
                     ...
                     exc=-(vy(i,j+1,k)*bz(i,j+1,k)
     +                    -vz(i,j+1,k)*by(i,j+1,k))
                     dexy=-g7(i,j,k)-exc

                  else   
                     

                     exc=-(vy(i,j+1,k+1)*bz(i,j+1,k+1)
     $                    -vz(i,j+1,k+1)*by(i,j+1,k+1))
                     dexy=-g7(i,j,k+1)-exc
     ...
     $                    -vx(i,j,k)*bz(i,j,k))
                     ...
                     exc=-(vz(i+1,j,k)*bx(i+1,j,k)
     $                    -vx(i+1,j,k)*bz(i+1,j,k))
     ...
     $                 -vx(i+1,j,k)*bz(i+1,j,k))
               ...
               if(abs(v2k).gt.epsk)then
                  ...
                  if(v2k.gt.0.)then
                     ...
                     exc=-(vz(i,j,k+1)*bx(i,j,k+1)
     $                    -vx(i,j,k+1)*bz(i,j,k+1))

                     dexy=-h5(i,j,k)-exc
                     
                  else 

                     exc=-(vz(i+1,j,k+1)*bx(i+1,j,k+1)
     $                    -vx(i+1,j,k+1)*bz(i+1,j,k+1))
     ...
     $                    -vy(i,j,k)*bx(i,j,k))
                     ...
                     exc=-(vx(i,j+1,k)*by(i,j+1,k)
     $                    -vy(i,j+1,k)*bx(i,j+1,k))
     ...
     $                 -vy(i,j+1,k)*bx(i,j+1,k))
               ...
               if(abs(v2k).gt.epsk)then
                  ...
                  if(v2k.gt.0.)then
                     ...
                     exc=-(vx(i+1,j,k)*by(i+1,j,k)
     $                    -vy(i+1,j,k)*bx(i+1,j,k))

                     dexy=-f6(i,j,k)-exc

                  else   

                     exc=-(vx(i+1,j+1,k)*by(i+1,j+1,k)
     $                    -vy(i+1,j+1,k)*bx(i+1,j+1,k))
...
c     close(10)
      ...
      Call Conserv(nxp,nyp,nzp,dn,vx,vy,vz,bx,by,bz,
     &     qu1,qu2,qu3,qu4,qu5,qu6,qu7)           
     ...
     &              +G7(I,J,K)-G7(I,J-1,K)+H7(I,J,K)-H7(I,J,K-1))
      ...
      Call Primitiv(nxp,nyp,nzp,qu1,qu2,qu3,qu4,qu5,qu6,qu7,
      ...
      Do k=k1,k2
         do n=0, nsubgrids-1
            Do j=j1,j2
               ...
               enddo
            enddo        
            if ( k .ge. fjstart(n*3+1)+1 .and. 
     $           k .le.   fjend(n*3+1)+1 ) then
      ...
      Do i=i1,i2
         do n=0, nsubgrids-1
            Do j=j1,j2
               ...
               enddo
            enddo        
            if ( i .ge. fistart(n*3+2)+1 .and. 
     $           i .le.   fiend(n*3+2)+1) then
      ...
      Do j=j1,j2
         do n=0, nsubgrids-1
            Do i=i1,i2
               ...
               enddo
            enddo                
            if ( j .ge. fjstart(n*3+3)+1 .and. 
     $           j .le.   fjend(n*3+3)+1) then
