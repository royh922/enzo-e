# 1 "/Users/juitenghsu/enzo-e/src/Enzo/hydro-mhd/ppm_fortran/calc_dt.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/Users/juitenghsu/enzo-e/src/Enzo/hydro-mhd/ppm_fortran/calc_dt.F"
...
# 4 "/Users/juitenghsu/enzo-e/src/Enzo/hydro-mhd/ppm_fortran/calc_dt.F" 2
c=======================================================================
...
# 49 "/Users/juitenghsu/enzo-e/src/Enzo/hydro-mhd/ppm_fortran/calc_dt.F" 2
c
c     Arguments
c
      integer idim, jdim, kdim, i1, i2, j1, j2, k1, k2, rank, ipfree
      real (kind=8) dx, dy, dz
      real (kind=8) dt, gamma, aye
      real (kind=8) d(idim,jdim,kdim), p(idim,jdim,kdim), u(idim,jdim,kdim),
...
c
      integer i,j,k
      real (kind=8) cs, dt1
C      real (kind=8) dmax,dmin,pmax,pmin,umax,umin,vmax,vmin
c
c\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\///////////////////////////////////
c=======================================================================
c
...
c
      if (rank .eq. 1) then
         do i = i1+1, i2+1
            cs = max(sqrt(gamma*p(i,1,1)/d(i,1,1)), 1.d-20)
            if (ipfree .eq. 1) cs = 1.d-20
...
c
      if (rank .eq. 2) then
         do j = j1+1, j2+1
            do i = i1+1, i2+1
               cs = max(sqrt(gamma*p(i,j,1)/d(i,j,1)), 1.d-20)
               if (ipfree .eq. 1) cs = 1.d-20
...
c
     ...
     &               write(6,*) 'calc_dt',d(i,j,k),p(i,j,k),i,j,k
                  cs = max(sqrt(gamma*p(i,j,k)/d(i,j,k)), 1.d-20)
                  if (ipfree .eq. 1) cs = 1.d-20
