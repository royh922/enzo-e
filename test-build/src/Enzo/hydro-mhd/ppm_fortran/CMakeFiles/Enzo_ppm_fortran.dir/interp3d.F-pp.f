# 1 "/Users/juitenghsu/enzo-e/src/Enzo/hydro-mhd/ppm_fortran/interp3d.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/Users/juitenghsu/enzo-e/src/Enzo/hydro-mhd/ppm_fortran/interp3d.F"
...
# 4 "/Users/juitenghsu/enzo-e/src/Enzo/hydro-mhd/ppm_fortran/interp3d.F" 2
...
# 52 "/Users/juitenghsu/enzo-e/src/Enzo/hydro-mhd/ppm_fortran/interp3d.F" 2
c
      integer MAX_REFINE, ijkn
      parameter (MAX_REFINE = 32, ijkn = (1024+6))
c
c-----------------------------------------------------------------------
c
c  arguments
...
c
C      print*, 'interp3d: ------------------------'
C      print*, 'interp3d: dim',dim1,dim2,dim3
C      print*, 'interp3d: start',start1,start2,start3
C      print*, 'interp3d: end',end1,end2,end3
C      print*, 'interp3d: refine',refine1,refine2,refine3
C      print*, 'interp3d: gdim',gdim1,gdim2,gdim3
C      print*, 'interp3d: gstart',gstart1,gstart2,gstart3
C      print*, 'interp3d: wdim',wdim1,wdim2,wdim3
...
c
               if (delf0 .eq. 0.0) then
                  delf0 = 1.d-5*sign
     &                 (1.d-20, (delf1 + delf2 + delf3))
                  sprime = 1.d0
               else
                  s = (delf1 + delf2 + delf3) / delf0
                  sprime = min(max(1.d0*s, 0.d0), 1.d0)
               endif
c
...
c
            do k1 = 0, refine3-1
               kc = gstart3+(k-1)*refine3+k1
               do j1 = 0, refine2-1
                  jc = gstart2+(j-1)*refine2+j1
                  do i1 = 0, refine1-1
                     do i = 1, pdim1
                        ic = gstart1+(i-1)*refine1+i1
                        grid(ic,jc,kc) = (fbar_save(i) +
     &                       (real(i1) + 0.5d0 - 
     &                       0.5d0*real(refine1))/
     &                       real(refine1)*fx(i) +
     &                       (real(j1) + 0.5d0 - 
