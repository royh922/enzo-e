# 1 "/Users/juitenghsu/enzo-e/src/Enzo/hydro-mhd/ppm_fortran/int_lin3d.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/Users/juitenghsu/enzo-e/src/Enzo/hydro-mhd/ppm_fortran/int_lin3d.F"
...
# 4 "/Users/juitenghsu/enzo-e/src/Enzo/hydro-mhd/ppm_fortran/int_lin3d.F" 2
...
# 53 "/Users/juitenghsu/enzo-e/src/Enzo/hydro-mhd/ppm_fortran/int_lin3d.F" 2
c
      integer MAX_REFINE
...
c
               delf0 = 0.5d0*(work(i+1, j+1, k+1) - work(i  , j  , k  ))
               delf1 = 0.5d0*(work(i  , j+1, k+1) - work(i+1, j  , k  ))
               delf2 = 0.5d0*(work(i+1, j  , k+1) - work(i  , j+1, k  ))
               delf3 = 0.5d0*(work(i+1, j+1, k  ) - work(i  , j  , k+1))
c
c              Enforce positivity if requested
c               (the value 0.8 is arbitrary, it should be in the range 0-1)
c
               if (iposflag .eq. 1) then
                  if (fbar + delf0 .lt. 0.0) delf0 = -0.8d0*fbar
                  if (fbar - delf0 .lt. 0.0) delf0 =  0.8d0*fbar
c
                  if (fbar + delf1 .lt. 0.0) delf1 = -0.8d0*fbar
                  if (fbar - delf1 .lt. 0.0) delf1 =  0.8d0*fbar
c
                  if (fbar + delf2 .lt. 0.0) delf2 = -0.8d0*fbar
                  if (fbar - delf2 .lt. 0.0) delf2 =  0.8d0*fbar
c
                  if (fbar + delf3 .lt. 0.0) delf3 = -0.8d0*fbar
                  if (fbar - delf3 .lt. 0.0) delf3 =  0.8d0*fbar
               endif
c
...
c
               if (delf0 .eq. 0.0) 
     &              delf0 = sign(1.d-20, delf0 + delf1 + delf2)
...
c
               ...
               chi2 = 1.d0
               chi3 = 1.d0
               if (delf1/delf0 .ge. 0.0) chi1 = 0.d0
               if (delf2/delf0 .ge. 0.0) chi2 = 0.d0
               if (delf3/delf0 .ge. 0.0) chi3 = 0.d0
c
...
c
     ...
     &                                    + (1.d0-chi2)*delf2
     &                                    + (1.d0-chi3)*delf3 ) /
     &           (chi1*delf1 + chi2*delf2 + chi3*delf3 + 1.d-20)
c
               frac = min(frac, 1.d0)
c
...
c
     ...
     &                       (real(i1) + 
     &                     0.5d0-0.5d0*real(refine1))/real(refine1)*fx +
     &                       (real(j1) +
     &                     0.5d0-0.5d0*real(refine2))/real(refine2)*fy +
     &                       (real(k1) +
     &                     0.5d0-0.5d0*real(refine3))/real(refine3)*fz )
