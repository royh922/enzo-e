# 1 "/Users/juitenghsu/enzo-e/src/Enzo/hydro-mhd/ppm_fortran/cic_interp.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/Users/juitenghsu/enzo-e/src/Enzo/hydro-mhd/ppm_fortran/cic_interp.F"
...
# 2 "/Users/juitenghsu/enzo-e/src/Enzo/hydro-mhd/ppm_fortran/cic_interp.F" 2
c=======================================================================
...
# 41 "/Users/juitenghsu/enzo-e/src/Enzo/hydro-mhd/ppm_fortran/cic_interp.F" 2
c
c-----------------------------------------------------------------------
c
...
c
     ...
     &        leftedge(3), cellsize
      real (kind=8)    sumfield(npositions), field(dim1, dim2, dim3)
c
c  locals
c
      integer i1, j1, k1, n
      real (kind=8) xpos, ypos, zpos, dx, dy, dz
      real (kind=8) half, edge1, edge2, edge3, fact
c
...
c
            sumfield(n) = sumfield(n) +
     &              field(i1  ,j1  ,1)*    dx *    dy  +
     &              field(i1+1,j1  ,1)*(1.-dx)*    dy  +
     &              field(i1  ,j1+1,1)*    dx *(1.-dy) +
     &              field(i1+1,j1+1,1)*(1.-dx)*(1.-dy)
c
         enddo
c
      endif
...
c 
     ...
     &           field(i1+1,j1  ,k1  ) * (1.-dx)*    dy *    dz +
     &           field(i1  ,j1+1,k1  ) *     dx *(1.-dy)*    dz +
     &           field(i1+1,j1+1,k1  ) * (1.-dx)*(1.-dy)*    dz +
     &           field(i1  ,j1  ,k1+1) *     dx *    dy *(1.-dz)+
     &           field(i1+1,j1  ,k1+1) * (1.-dx)*    dy *(1.-dz)+
     &           field(i1  ,j1+1,k1+1) *     dx *(1.-dy)*(1.-dz)+
     &           field(i1+1,j1+1,k1+1) * (1.-dx)*(1.-dy)*(1.-dz)
c
