# 1 "/Users/juitenghsu/enzo-e/src/Enzo/gravity/grid_cic.F"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/Users/juitenghsu/enzo-e/src/Enzo/gravity/grid_cic.F"
...
# 2 "/Users/juitenghsu/enzo-e/src/Enzo/gravity/grid_cic.F" 2
c=======================================================================
...
# 52 "/Users/juitenghsu/enzo-e/src/Enzo/gravity/grid_cic.F" 2
c
c-----------------------------------------------------------------------
c
...
c
      ...
      real (kind=8)   fact1, fact2, fact3, x, y, z, dx, dy, dz,
     &     weight, mass,
...
c
         do i=1,ddim1
            shift1 = temp(i,1,1,2)/max(temp(i,1,1,1), 1.d-20) * coef1
...
c
         do j=1, ddim2
            do i=1, ddim1
               shift2 = temp(i,j,1,3)/
     &                  max(temp(i,j,1,1),1.d-20)*coef2
               y = min(max((j - 0.5e0 + shift2), half), edge2)
               j1 = int(y + 0.5e0)
               dy = REAL(j1,4) + 0.5e0 - y
               shift1 = temp(i,j,1,2)/max(temp(i,j,1,1),1.d-20)*coef1
               x = min(max((i - 0.5e0 + shift1), half), edge1)
               i1 = int(x + 0.5e0)
               dx = REAL(i1,4) + 0.5e0 - x
               ox = (1.e0-dx)
...
c
         do k=1, ddim3
            do j=1, ddim2
               do i=1, ddim1
                 shift3 = temp(i,j,k,4)/max(temp(i,j,k,1),1.d-20)*coef3
                  z = min(max((k - 0.5e0 + shift3), half), edge3)
                  k1 = int(z + 0.5e0)
                  dz = REAL(k1) + 0.5e0 - z
c
                 shift2 = temp(i,j,k,3)/max(temp(i,j,k,1),1.d-20)*coef2
                  y = min(max((j - 0.5e0 + shift2), half), edge2)
                  j1 = int(y + 0.5e0)
                  dy = REAL(j1) + 0.5e0 - y
c
                 shift1 = temp(i,j,k,2)/max(temp(i,j,k,1),1.d-20)*coef1
                  x = min(max((i - 0.5e0 + shift1), half), edge1)
                  i1 = int(x + 0.5e0)
                  dx = REAL(i1) + 0.5e0 - x
c
