 ...
 const std::array<enzo_float,3> cell_widths_xyz) const noexcept
{
  ...
  EnzoSourceDiffusion diffusion_source(viscosity_nu_, thermal_kappa_);
  diffusion_source.calculate_source(cur_dt, primitive_map, dUcons_map,
                                     cell_widths_xyz, stale_depth);
