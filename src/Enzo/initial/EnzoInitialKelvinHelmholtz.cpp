...
void EnzoInitialKelvinHelmholtz::enforce_block(Block* block, const Hierarchy* hierarchy) throw() {
    ...
    bool has_bfield = (field.is_field("bfield_x") && field.is_field("bfield_y") && field.is_field("bfield_z"));
    if (has_bfield) {
    ...
    const double p_uniform =
        rho_cold * k_B * T_cold / (mol_weight * m_H) / std::pow(enzo_units->length() / enzo_units->time(), 2);
    ...
    const double c_s = std::sqrt(gamma_adi_ * p_uniform / rho_cold);  // Cold stream sound speed
    ...
    if (has_bfield) {
        magnetic_edens = 0.5 * (uniform_bfield_[0] * uniform_bfield_[0] + uniform_bfield_[1] * uniform_bfield_[1] +
    ...
    for (int iz = 0; iz < mz; iz++) {
        ...
        for (int iy = 0; iy < my; iy++) {
            ...
            for (int ix = 0; ix < mx; ix++) {
                ...
                // Add perturbations at the interface according to Mandelker et al. (2020)
                // v_yz^pert (r, \phi, z) = \sum_k v_0 \cos(2 \pi n_j z + m_j \phi + \Phi_j) *
                //                          exp[-(r - R_s)^2 / (2 \sigma^2)]
                // v_0 = 0.005 * c_s, n_j = integer in [2, 64], m_j = integer in [0, 1]
                // \Phi_j = random phase, \sigma = R_s / 16

                if (std::abs(r - radius_) < radius_ / 8) {  // Near the interface
			    ...
			    mode_sum += std::cos(2.0 * pi * k * x / 32.0 + m * phi);
                    ...
                    vel_z += mag * mode_sum * std::cos(phi);
                    vel_y += mag * mode_sum * std::sin(phi);
                }
